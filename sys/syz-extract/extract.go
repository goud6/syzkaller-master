...
func main() {
	...
	//1.调用archFileList函数得到我们编写的系统调用模板文件和相应的架构的配置。
	...
	//2.调用prepare函数，首先检查是否设置了sourcedir源代码目录，
	//如果设置build参数表示重新生成特定架构的内核头文件于是make mrproper清理；
	//如果没有设置build参数则不能指定多个架构。
    ...
    //对每个运行时进程执行worker（processArch和processFile）
	...
	for file, cf := range constFiles {
		...
		//最后调用Serialize函数将其序列化为常量=值的形式，最后写到文件。
		data := cf.Serialize()
		if len(data) == 0 {
			os.Remove(outname)
			continue
...
func archFileList(os, arch string, files []string) (string, []string, []string, error) {
	...
	//这是linux特有的，实际应该移动到linux.go文件中
	android := false
	if os == "android" {
		android = true
		os = targets.Linux
	}
	var arches []string    //切分架构
	if arch != "" {      //参数中有关于架构的信息，就从参数中获得，否则从OS的信息中响应OS配置的arch
		arches = strings.Split(arch, ",")
	} else {
		for arch := range targets.List[os] {   //从
			arches = append(arches, arch)
		}
		if android {
...
func processArch(extractor Extractor, arch *Arch) (map[string]*compiler.ConstInfo, error) {
	...
	//1.调用ParseGlob函数，ParseGlob函数调用了Parse函数将编写的txt文件解析成AST， top是AST的顶节点。
	top := ast.ParseGlob(filepath.Join("sys", arch.target.OS, "*.txt"), eh)
	if top == nil {
		return nil, fmt.Errorf("%v", errBuf.String())
	}
	//2.调用ExtractConsts函数提取出常量标识符，ExtractConsts函数主要调用了Compile函数。
	infos := compiler.ExtractConsts(top, arch.target, eh)
	if infos == nil {
		return nil, fmt.Errorf("%v", errBuf.String())
	}
	//返回到processArch函数，将文件名和对应的常量标识符信息发给Channel之后就可以调用processFile函数了。
