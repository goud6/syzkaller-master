...
const (
	...
	//Triage所有hub中新的输入，这时我们开始复现crash
	phaseTriagedHub
)
...
func main() {
	...
	flag.Parse()
	//2.指定日志大小，不能超过1000行或者1^20字节
	log.EnableLogCaching(1000, 1<<20)
	//3.加载config文件，获取操作系统和架构信息，检查是否支持
...
func RunManager(cfg *mgrconfig.Config) {
	...
	//在RunManager函数中如果config文件中指定的type不为none则创建一个vmpool。
	//将type指定为none是在调试/开发中用的，这样manager就不会启动VM而是需要手动启动。
	if cfg.Type != "none" {
		var err error
		//一个vmPool可以用来创建多个独立的VM。
