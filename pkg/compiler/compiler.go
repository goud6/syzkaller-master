...
func createCompiler(desc *ast.Description, target *targets.Target, eh ast.ErrorHandler) *compiler {
	...
	//在syscall_descriptions_syntax.md中可以看到syzkaller内建的一些别名和模板，在createCompiler函数中对它们进行了初始化。
...
// Compile 编译系统的description。
func Compile(desc *ast.Description, consts map[string]uint64, target *targets.Target, eh ast.ErrorHandler) *Prog {
	//1. Compile函数中首先调用的是createCompiler函数。在syscall_descriptions_syntax.md中可以看到syzkaller内建的一些别名和模板，在createCompiler函数中对它们进行了初始化。
	...
	if consts == nil {
		//3. extractConsts函数返回提取const值所需的文本常量和其它信息的列表。
