...
type Pos struct {
	File string
	Off  int // byte offset, starting at 0, 比特偏移量，从0开始
	Line int // line number, starting at 1   //行号，从1开始
	Col  int // column number, starting at 1 (byte count)   //列数，从一开始（bit计数）
}

// Description contains top-level nodes of a parsed sys description.
//Description 包含已解析sys描述的顶级节点。
...
type Node interface {
	...
	// walk为此节点的所有子节点调用回调函数cb。它不是递归的。使用递归helper来进行递归 walk。
	walk(cb func(Node))
}
